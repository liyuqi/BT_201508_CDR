...
//mongo cdr --port 40000 "./gprs_fields2.js" > ./gprs_field$(date +"%Y%m%d")_$(date +"%H%M%S").txt
...
for(var i =0;i<gprs_field.length;i++) {
    //var tempagg =
        db.getCollection('gprsdistinct_'+gprs_field[i].toString()).find({},{})
        .forEach(function (doc){
                var tempdoc = doc;
                //tempdoc._id = new ObjectId();
                tempdoc._id = gprs_field[i];
