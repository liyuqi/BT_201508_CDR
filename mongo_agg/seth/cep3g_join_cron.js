...
//mongo cdr --eval "var bucket_size=100, n="+%n+", oid="+%oid+";" cep3g_join.js > ./cep3g_join_result_$(date +"%Y%m%d")_$(date +"%H%M%S").txt

//var max_id=ObjectId("552cca5a0cf2e0d926e64d38"), bucket_size=100000, n=1
...
var cdr3g = db.cep3g_gen1.find({	//seth 測試用
...
//    ,up_flag : {$nin:[1]}
    ...
    ,"up_flag" : 1
    ,"_id":1
}).addOption(DBQuery.Option.noTimeout).sort({_id:1}).limit(bucket_size).skip((n-1)*bucket_size).forEach(function (doc) {	    //seth 改
//while(cdr3g.hasNext()){
    //var doc = col.next();
...
//        db.cep3g_sample.update({_id: doc._id}, {$set: {up_flag:1}});
        db.cep3g_gen1.update({_id: doc._id}, {$set: {up_flag:1}});		//seth 測試用
        db.cep3g_join1.save(doc);	//seth 測試用
	print(doc._id)
    }else{}
    i++;
});
...
db.cep3g_gen1.findOne({up_flag:1},{time:1,up_flag:1});
